...
type UserRole = "free" | "starter" | "pro"
...
const HOOKS: Hook[] = [
  ...
  { id: 3,  category: "Chiffres",     text: "En 90 jours, j'ai triplé mon TJM. La méthode étape par étape.", premium: true },
  ...
  { id: 8,  category: "Question",     text: "Qu'est-ce qui différencie un freelance à 300€/j d'un freelance à 800€/j ?", premium: true },
  // Storytelling
  { id: 9,  category: "Storytelling", text: "Il y a 18 mois, j'étais en CDI avec un salaire fixe. Aujourd'hui je gagne 2x plus. Ce qui a tout changé :", premium: true },
  ...
  { id: 14, category: "Controverse",  text: "Arrêtez de baisser vos tarifs. C'est le pire conseil qu'on puisse vous donner.", premium: true },
  ...
  { id: 19, category: "Curiosité",    text: "J'ai analysé 200 posts LinkedIn viraux. Voici le pattern.", premium: true },
  { id: 20, category: "Curiosité",    text: "Le secret des freelances qui gagnent bien sans travailler plus.", premium: true },
]
...
// Premier hook par catégorie = visible pour le plan gratuit
const FREE_VISIBLE_IDS = new Set(
  (["Chiffres", "Question", "Storytelling", "Controverse", "Curiosité"] as Category[]).map(
    (cat) => HOOKS.find((h) => h.category === cat)!.id
  )
)

function getHookLock(hook: Hook, role: UserRole): { locked: boolean; message: string } | null {
  if (role === "pro") return null
  if (role === "starter") {
    if (!hook.premium) return null
    return { locked: true, message: "Passez au Pro pour débloquer les hooks premium" }
  }
  // free
  if (FREE_VISIBLE_IDS.has(hook.id)) return null
  return { locked: true, message: "Passez au Starter pour débloquer tous les hooks" }
}

/* ─── Page ──────────────────────────────────────────────────── */

export default function HooksClient({ role }: { role: UserRole }) {
    ...
    <div className="space-y-6">
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((hook) => {
            const lock = getHookLock(hook, role)
            return (
              <div
                key={hook.id}
                className="relative bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 flex flex-col gap-4 shadow-sm hover:shadow-md hover:border-slate-200 dark:hover:border-slate-600 transition-all overflow-hidden"
              >
                {/* Contenu flouté si verrouillé */}
                <div className={lock ? "blur-sm pointer-events-none select-none" : ""}>
                  {/* Badge catégorie + badge premium */}
                  <div className="flex items-center gap-2 mb-4">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${CATEGORY_STYLES[hook.category].badge}`}>
                      {hook.category}
                    </span>
                    {hook.premium && (
                      <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300">
                        ✨ Premium
                      </span>
                    )}
                  </div>
                  ...
                  {/* Texte du hook */}
                  <p className="text-sm text-slate-800 dark:text-slate-200 leading-relaxed flex-1 mb-4">
                ...
                {lock && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 dark:bg-slate-800/80 backdrop-blur-[2px] rounded-2xl">
                    <span className="text-2xl mb-2">🔒</span>
                    <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 text-center px-6 leading-snug">
                      {lock.message}
                    </p>
                    <Link
                      href="/billing"
